      ...
      <div className="grid grid-cols-4 sm:grid-cols-7 gap-1.5">
        {QUICK_AMOUNTS.map((q) => (
          <button
            ...
            className="px-2 py-1.5 rounded-lg text-xs font-orbitron border disabled:opacity-30 disabled:cursor-not-allowed transition-all"
            style={{
        ...
        <button
          ...
          disabled={disabled}
          onClick={setMax}
          className="px-2 py-1.5 rounded-lg text-xs font-orbitron font-bold border border-[var(--gold-dim)] text-[var(--gold)] hover:bg-[rgba(255,210,74,0.1)] disabled:opacity-30 disabled:cursor-not-allowed transition-all"
        >
          MAX
        </button>
      </div>
      ...
      <div className="flex gap-2">
        ...
        >
          ...
          <input
            ...
            max={maxEnergy}
            disabled={disabled}
            className="flex-1 bg-transparent px-3 py-2 text-sm text-[var(--text-primary)] placeholder-[var(--text-muted)] focus:outline-none disabled:opacity-40 disabled:cursor-not-allowed"
          />
        </div>
        <button
          type="submit"
          disabled={!isValid || !choice || disabled || loading}
          className="px-6 py-2 rounded-lg font-orbitron text-xs font-bold tracking-widest text-black disabled:opacity-30 disabled:cursor-not-allowed transition-all"
          style={{
